    ...
    class Short < Pancake::Stack
      ...
      class << self
        ...
        def define_action(method, path, block)
          ...
          attach_route(method, path, action_name)
        end
        ...
        # @api private
        # @author Daniel Neighman
        def attach_route(method, path, action_name)
          self.add_routes do |r|
            r.map(path, method, :to => Controller, :with => {:action => action_name}, :anchor => true)
          end
        end
